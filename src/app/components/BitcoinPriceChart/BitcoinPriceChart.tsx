...
// {
//     "usd": 92649,
//     "usd_24h_change": -0.11723462903747844,
//     "inr": 7979071,
//     "inr_24h_change": 0.157855275940101
// } 
const formatCurrency = (amount:number, currency : string) => {
  ...
  return (
    <div className="bg-white rounded-lg flex px-[24px] pb-[31px] xl:pb-[20px] pt-[24px]  flex-col   h-[711px]  md:w-[63%]  ">
      {/* //// header  */}
      <div className="   ">
        <div className="flex items-center gap-10 ">
          <div className="flex gap-2 items-center">
            ...
            >
              <rect width={36} height={36} fill="url(#pattern0_1_514)" />
              <defs>
                <pattern
                  id="pattern0_1_514"
            ...
            <span className="text-[#0B1426] text-[24px] leading-[28px] font-semibold ">
              Bitcoin
            </span>
            <span className="text-[#5D667B] mt-[5.5px] ">BTC</span>
          </div>
          <button className=" bg-[#768396] border border-[#808A9D] w-[80px] h-[40px] rounded-lg text-white ">
            Rank #1
          </button>
        </div>
      </div>
        ...
        <div className="h-[66px]  mb-[24px]  flex gap-[32px]">
          <div className="flex flex-col">
            <span className="text-[#0B1426] font-semibold text-[28px] ">
              {isLoading ? "0" : formatCurrency(dataObj?.usd, "USD")}
            </span>
            <span className="text-[#0B1426]">
              {isLoading ? "0" : formatCurrency(dataObj?.inr, "INR")}
            </span>
          </div>
          <div className="flex mt-2 gap-3">
            <span
              className={`${
                Number(dataObj?.usd_24h_change.toFixed(2)) > 0
                  ? "bg-[#EBF9F4] text-[#14B079] "
                  : "bg-[#e46060] text-white"
              }  font-medium rounded h-[28px] text-[14px] w-[84px] flex items-center justify-center gap-2 `}
            >
              {Number(dataObj?.usd_24h_change.toFixed(2)) > 0 ? (
                <svg
                  width="11"
                ...
                <svg
                  ...
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path d="M5.5 0L11 8H0L5.5 0Z" fill="#8b1a1a" />
                </svg>
              )}
              {isLoading ? "0 %" : dataObj?.usd_24h_change.toFixed(2)} %
            </span>
